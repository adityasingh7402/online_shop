...
export default function Home({ orders, randomNum, selectUser, selectUsers, winnOrder, withdrawals, addcoins, userss, Querys }) {
  ...
  const [active, setActive] = useState(""); // Default to an empty string for the front page
  ...
  return (
    <div className="overflow-hidden h-screen containerr">
      ...
      />
      <Link href={'/'}>
        <div className="gotoHome right-10 top-10 fixed cursor-pointer text-white p-2 bg-red-900 font-bold text-4xl">
          <AiOutlineClose />
        </div>
      </Link>
      <div className="my-8 px-5 w-full flex flex-row">
        <div className="bg-red-800 w-56 border border-gray-200 rounded-sm py-8 px-5 shadow-sm h-min">
          <ul className="flex flex-col">
            <li className="text-xl pb-5 font-medium text-white flex items-center">
              <span className="text-2xl pr-5"><HiChartPie /></span> Inventory
            </li>
            <Link href={"/admin/Users"}>
              <li className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
                <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Users
              </li>
            </Link>
            <li onClick={() => setActive("Order")} className="text-base font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
              <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Orders
            </li>
            <div className="line-braker h-1 w-full bg-white rounded-full my-10"></div>
            <li onClick={() => setActive("ChangeNum")} className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
              <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Change Card No
            </li>
            <li onClick={() => setActive("WinnerSelect")} className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
              <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Winner Select
            </li>
            <Link href={"/admin/Withdrawal"}>
              <li className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
                <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Withdrawal Coins
              </li>
            </Link>
            <Link href={"/admin/AddCoin"}>
              <li className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
                <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Adding Coins
              </li>
            </Link>
            <li onClick={() => setActive("Queryy")} className="text-base pb-5 font-medium cursor-pointer hover:text-red-200 text-white flex items-center">
              <span className="text-xl pr-5"><RiSendPlane2Fill /></span> Query
            </li>
          </ul>
          <ul className="flex flex-col mt-2">
            <Link href={'/'}>
              <li className="text-base pb-4 cursor-pointer hover:text-red-200 text-white flex items-center">
                <button className="px-5 py-2 border broder-white w-full rounded-full hover:bg-red-900"><p>Logout</p></button>
              </li>
            </Link>
          </ul>
        </div>
        <div className="bg-white w-5/6 text-sm text-gray-800 ml-5 border border-gray-200 rounded-sm py-3 px-2 shadow-sm overflow-scroll h-screen">
          {active === "" && ( // Render Admin Panel text only when active state is empty (front page)
            <div className="admin-pannel flex justify-center items-center h-full">
              <div className="clasimg font-serif text-6xl font-semibold animate-charcter m-auto">
                Admin Panel
              </div>
            </div>
          )}
